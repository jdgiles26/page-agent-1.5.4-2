...
function getAutonomousAgentLine(useCN?: boolean) {
	const cdn = useCN ? CDN_DEMO_CN_URL : CDN_DEMO_URL

	return `javascript:(function(){var s=document.createElement('script');s.src='${cdn}?t='+Math.random();s.crossOrigin='anonymous';s.type='text/javascript';s.onload=()=>console.log('PageAgent ready!');document.body.appendChild(s);})();`
}
...
export default function HeroSection() {
	...
	const oneLiner = getAutonomousAgentLine(cdnSource === 'china')
	const oneLinerHref = encodeURI(oneLiner)
								...
								<div className="p-4">
									...
									{activeTab === 'other' && (
										...
										<div className="bg-green-50 dark:bg-gray-700 p-4 rounded-lg">
											<div className="flex items-center justify-center gap-2 mb-3 text-left">
												<span className="inline-flex items-center text-xs px-3 py-1 rounded-full bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-700/60">
													{isZh ? '一行启动脚本已就绪' : 'Autonomous browser agent line ready'}
												</span>
												<span className="inline-flex items-center gap-2 text-xs px-2 py-1 rounded-full bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 border border-blue-200 dark:border-blue-700/60">
													<span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse" />
													{cdnSource === 'china' ? 'npmmirror' : 'jsdelivr'}
												</span>
											</div>
											<p className="text-gray-700 dark:text-gray-300 text-sm mb-3">
											...
											<div className="flex items-center justify-center gap-3">
												...
												<a
													...
													target="_blank"
													className="inline-flex items-center text-xs px-3 py-2 bg-blue-500 text-white font-medium rounded-lg hover:shadow-md transform hover:scale-105 transition-all duration-200 cursor-move border-2 border-dashed border-green-300"
													title="Drag me to your bookmarks bar"
												>
													✨ PageAgent Bookmark
												</a>
											</div>
											<p className="text-[11px] text-left text-gray-600 dark:text-gray-300 mt-3">
												{isZh
													? '或直接复制以下一行到收藏夹链接（更快）：'
													: 'Or copy this one line into the bookmark URL:'}
											</p>
											<div className="relative">
												<input
													value={oneLiner}
													readOnly
													className="w-full px-3 py-2 text-xs font-mono bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-500 rounded-md text-gray-700 dark:text-gray-100 pr-16"
													onFocus={(e) => e.currentTarget.select()}
												/>
												<button
													onClick={handleCopyOneLiner}
													className="absolute right-1 top-1 px-3 py-1 text-[11px] rounded bg-blue-600 text-white hover:bg-blue-700 transition-colors"
												>
													{copyState === 'copied'
														? (isZh ? '已复制' : 'Copied')
														: copyState === 'error'
